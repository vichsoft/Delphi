...
function DESDecryptStr(Str: TBytes; Key: string; ResultEncoding: TEncoding = nil;
  KeyEncoding: TEncoding = nil;
...
function DESEncryptStrToHex(Value, Key: string; StrEncoding: TEncoding = nil;
  KeyEncoding: TEncoding = nil;
  ValueCRLFMode: TCRLFMode = rlCRLF;
  KeyCRLFMode: TCRLFMode = rlCRLF;
  OnProcessProc: TOnProcessProc = nil;
...
function DESDecryptStrFromHex(StrHex, Key: string; ResultEncoding: TEncoding = nil;
  KeyEncoding: TEncoding = nil;
  ResultCRLFMode: TCRLFMode = rlCRLF;
  KeyCRLFMode: TCRLFMode = rlCRLF;
  OnProcessProc: TOnProcessProc = nil;
...
//    end;
    ...
    try
      DESEncryptStream(InStream, OutStream, 0, Key, KeyEncoding, OnProcessProc, ProcessProc);
...
begin
  ...
  j := Length(KeyBytes);
  if Length(KeyBytes) < 8 then
  begin
    SetLength(KeyBytes, 8);
    for I := j to 7 do
      KeyBytes[I] := 0;
  end;
  for j := 0 to 7 do KeyByte[j] := Ord(KeyBytes[j]);
  CurrDone := 1;
  if Assigned(OnProcessProc) then
    OnProcessProc(nil, ProcMin, ProcMax, CurrDone, Cancel);
  if Assigned(ProcessProc) then
